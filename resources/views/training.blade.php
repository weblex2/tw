<x-guest-layout>
    <div class="main-content">
    <div class="content-wrapper training">
        <div class="content flex justify-center">

            <div class="grid grid-cols-1 w-[75%] lg:ml-20 lg:grid-cols-3">

                <div class="lg:col-span-3 tt">
                    <div class="h1">Trainingszeiten</div>
                </div>

                <div class="tt">
                    <div class="w-full">
                        <div class="h2">PEEWEES</div>
                    </div>
                    <h3>Trainingszeiten</h3>
                    <div>
                        <div>Mittwoch: 17.30 – 19.30 Uhr</div>
                        <div>Gemeindehalle Feldkirchen</div>
                        <div>Richthofenstrasse 1</div>
                        <div>85622 Feldkirchen</div>
                        <br/>
                        <div>Sonntag: 17.00 – 19.00 Uhr</div>
                        <div>Dreifachturnhalle Feldkirchen</div>
                        <div>Olympiastr. 3</div>
                        <div>85622 Feldkirchen</div>
                    </div>
                </div>
                <div class="tt">
                    <h2>JUNIORS</h2>
                    <h3>Trainingszeiten</h3>
                    <div>
                        <div>Montag: 18.00 – 20.00 Uhr</div>
                        <div>Dreifachturnhalle Feldkirchen</div>
                        <div>Olympiastr. 3</div>
                        <div>85622 Feldkirchen</div>
                        <br/>
                        <div>Mittwoch: 17.30 – 19.30 Uhr</div>
                        <div>Gemeindehalle Feldkirchen</div>
                        <div>Richthofenstrasse 1</div>
                        <div>85622 Feldkirchen</div>
                        <br/>
                        <div>Sonntag: 17.00 – 19.00 Uhr</div>
                        <div>Gemeindehalle Feldkirchen</div>
                        <div>Richthofenstrasse 1</div>
                        <div>85622 Feldkirchen</div>
                    </div>
                </div>
                <div class="tt">
                    <h2>SENIORS</h2>
                    <h3>Trainingszeiten</h3>
                    <div>
                        <div>Montag: 20.00 – 22.00 Uhr</div>
                        <div>Dreifachturnhalle Feldkirchen</div>
                        <div>Olympiastr. 3</div>
                        <div>85622 Feldkirchen</div>
                        <br>
                        <div>Mittwoch: 19.30 – 21.30 Uhr</div>
                        <div>Gemeindehalle Feldkirchen</div>
                        <div>Richthofenstrasse 1</div>
                        <div>85622 Feldkirchen</div>
                        <br>
                        <div>Sonntag: 19.00 – 21.00 Uhr</div>
                        <div>Gemeindehalle Feldkirchen</div>
                        <div>Richthofenstrasse 1</div>
                        <div>85622 Feldkirchen</div>
                    </div>
                </div>

            </div>

        </div>
    </div>
    </div>
</x-guest-layout>
